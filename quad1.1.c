...
void clearBuffer();
int solveLinear(int a, int b, int c, double *x1);
...
void finalOutput(double * x1, double*x2,
    ...
    switch(nRoots){
        ...
        case INFINITY_OF_ROOTS:
            ...
            break;
        default: printf("Произошла ошибка, кол-во корней — %d\n", nRoots);
    }
}
// очистка буфера
void clearBuffer(){
...
int solveLinear(int a, int b, int c, double * x1){
    // (x-0) < INFELICITY — проверка x на равенство 0
    if((b - 0) < INFELICITY){
        if((c - 0) < INFELICITY){
